...
internal class Web3Service {
    ...
    companion object {
        ...
        fun newAccount(): String {
            ...
            var credentials = Credentials.create(Hex.toHexString(bytes))
            val file = File(walletFileDirectory, ".key")
            if (!file.exists()) {
                file.createNewFile()
            } else {
                // TODO
                //throw Exception("Keypair already exists on device!")
            }
    ...
    class HelloWorld {
        ...
        companion object {
            ...
            fun addListener(listener: EventListener) {
                ...
                val dataThread = DatabaseService.DataThread("Hello World")
                dataThread.start()
